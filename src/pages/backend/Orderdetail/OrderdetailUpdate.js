...
function OrderdetailUpdate() {
    ...
    async function OrderdetailUpdate(event) {
    ...
    useEffect(function () {
        (async function () {
            await OrderdetailService.getById(id).then(function (result) {
                const ord = result.data.orderdetail;
                setOrder_id(ord.order_id);
                setProduct_id(ord.product_id);
                setPrice(ord.price);
                setQty(ord.qty);
                setAmount(ord.amount);
    ...
    return (<>
         <form method="post" onSubmit={OrderdetailUpdate}>
            <div className="card">
